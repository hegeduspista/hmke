    ...
    class Root(Tk):
        ...
        def buttons(self):
            ...
            self.button = ttk.Button(
                self.labelFrame,
                text="Fájl megnyitása és az adatbázis frissítése",
        ...
        def fajlnev(self):
            ...
            try:
                ...
                if xlsx_fajl_neve_darabolva[-1] != "Tűzesetis.xlsx":
                    ...
                    tuzes = 0
                    uj_falj_nev = (
                        xlsx_fajl_neve_darabolva[1]
                        + "_"
                        + xlsx_fajl_neve_darabolva[4]
                        + ".xlsx"
                    )
                ...
                if path.isfile(mentes_helye + tn) and path.isfile(mentes_helye + t):
                    ...
                    if (
                        xlsx_fajl_neve_darabolva[1] == "GROWATT"
                        and xlsx_fajl_neve_darabolva[4] == "295"
                    ):
                        tableBaseName = "G295CS"
                    if (
                        xlsx_fajl_neve_darabolva[1] == "GROWATT"
                        and xlsx_fajl_neve_darabolva[4] == "315"
                    ):
                        tableBaseName = "G315HY"
                    if (
                        xlsx_fajl_neve_darabolva[1] == "FRONIUS"
                        and xlsx_fajl_neve_darabolva[4] == "295"
                    ):
                    ...
                    if (
                        xlsx_fajl_neve_darabolva[1] == "SOLAREDGE"
                        and xlsx_fajl_neve_darabolva[4] == "295"
                    ):
                    ...
                    read_CSV.to_sql(
                        tableBaseName, conn, if_exists="replace", index=False
                    )
...
def user_controll():
    ...
    mac = get_mac_address(network_request=False)
    ...
    if not ok:
        ...
        if mac == tarolt_mac:
            ...
            lejarat_10 = datetime.date(
                int(lejar[0]), int(lejar[1]), int(lejar[2])
            ) - datetime.timedelta(days=10)
            lejarat_10_plus = datetime.date(
                int(lejar[0]), int(lejar[1]), int(lejar[2])
            ) + datetime.timedelta(days=10)
            ...
            if lejarat < current_date < lejarat_10_plus:
                messagebox.showinfo(
                    "Figyelmeztetés",
                    "A licensz szerződés lejárt "
                    + str(10 - int(lejart[0]))
                    + " napja!\n"
                    + str(lejarat)
                    + "\nMég "
                    + str(lejart[0])
                    + " nap van a szerződés megújítására!",
                )
            if current_date > lejarat_10_plus:
                messagebox.showerror(
                    "Figyelmeztetés", "A program szerződése lejárt!")
                sql2 = (
                    "UPDATE felhasznalok SET jogosultsag='0' WHERE mac like '%"
                    + mac
                    + "%'"
                )
                cur.execute(sql2)
...
kristaly = [
    "Monokristályos (Growatt 315)",
    "Monokristályos (Fronius 315)",
    "Monokristályos (SolarEdge 315)",
    "Polikristályos (Fronius 295)",
    "Polikristályos (Growatt 295)",
    "Polikristályos (SolarEdge 295)",
]
...
v_szamla_input = Entry(adatbekeres_frame, width=25, justify="center")
...
Label(
    adatbekeres_frame, text="Megye:", font=text_font, bg=bg, fg=fg, width=23, anchor="w"
).grid(row=3, column=0, padx=10, pady=5)
megye_combo = Combobox(adatbekeres_frame, width=25,
                       justify="center", state="readonly")
# megye_combo.bind("<<ComboboxSelected>>", jaras_nev)
...
def rendszer_meret():
    global aramdij, eves_kwh, panel_meret, panel_szam, poli_mono_kWp, tajolas, polikristalyos_kWp, pvgis, dolesszog, teljesitmeny_sz
    v_szamla_input.focus()
    ...
    try:
        ...
        sql = (
            "SELECT teljesitmeny FROM teljesitmeny_leadas WHERE "
        ...
        if (
            tajolas_combo.get() != "válasszon tájolást"
            and teto_dolesszog_combo.get() != "válasszon dőlésszöget"
            and szolgaltato_combo.get() != "válasszon áramszolgáltatót"
            and megye_combo.get() != "válasszon megyét"
        ):
            kezdo_kep.grid_forget()
            kezdo_kep2.grid_forget()
            adatok_frame.grid(
                row=0, column=1, columnspan=2, rowspan=8, sticky="n", padx=5, pady=3
            )
        ...
        # tulmeretez = tulmeretezes.get()

        poli_mono_szetszedve = str(poli_mono.get()).split(" ")

        # telj = str(poli_mono_szetszedve[1]).split(' ')
        ...
        # eves_termeles_1kw_rendszerre = pvgis * teljesitmeny_sz

        rendszer_merete = round(float(szuks_rendszer_telj / 1000), 2)
        # panel_szam = round(rendszer_merete / poli_mono_kWp)
...
def select_ajanlott_inverter(egysor):
    global pdf_csomag
    # print(minden_ajanlat[egysor])
    csomagok = osszes_ajanlat[egysor]
    # if panel_szam == csomagok[2]:
    inverter.set("")
...
def popupmsg():
    ...
    try:
        ...
        popup.geometry(
            str(popup_width) + "x" + str(popup_height) + "+" + popup_location
        )
        # win.resizable(width=False, height=False)
        popup.configure(bg="green")
        # ajánlatkérő adatainak bekérése
        bg1 = "green"
        Label(popup, text="", bg=bg1, fg=fg, width=15, anchor="w", font=text_font).grid(
            row=0, column=0, columnspan=2, sticky="nsew", padx=10, pady=12
        )

        Label(
            popup,
            text="Ajánlatkérő neve:",
            bg=bg1,
            fg=fg,
            width=15,
            anchor="w",
        ...
        Label(
            popup, text="Lakcíme:", bg=bg1, fg=fg, width=10, anchor="w", font=text_font
...
def napelemes_rendszer(rm, mrmt, panelsz, teljesitmeny_szazalek):
    ...
    # print(rm)
    # print(adatbazis_tabla, poli_mono.get())

    if "MFB pályázat" in finansz:
        max_panelszam = panelsz + 1
        # print(adatbazis_tabla)
        if tulmeretezes.get() == "Igen":
            sql = (
                "SELECT * FROM '{0}' WHERE {1}<=napelemszam AND napelemszam<={2} AND FSZ={3} AND TUZESETI={4}"
                " AND SZEZONALIS={5} AND mfbAR='Igen' AND mfbMuszak='Igen' ORDER BY rm LIMIT 6".format(
                    adatbazis_tabla,
                    str(panelsz),
                    str(max_panelszam),
                    str(fazis),
        ...
        if tulmeretezes.get() == "Nem":
            # mrmtt = round(float(rm * 1.1), 2)
            sql = (
                "SELECT * FROM '{0}' WHERE {1}=napelemszam AND FSZ={3} AND TUZESETI={4} AND SZEZONALIS={5} "
                "AND mfbAR='Igen' AND mfbMuszak='Igen' ORDER BY rm LIMIT 6".format(
                    adatbazis_tabla,
                    str(panelsz),
                    str(max_panelszam),
                    str(fazis),
    ...
    if "MFB pályázat" not in finansz:
        ...
        if tulmeretezes.get() == "Igen":
            ...
            sql = (
                "SELECT * FROM '{0}' WHERE {1}<=napelemszam AND napelemszam<={2} AND FSZ={3} AND TUZESETI={4} "
                "AND SZEZONALIS={5} ORDER BY rm LIMIT 6".format(
                    adatbazis_tabla,
                    str(panelsz + 1),
                    str(max_panelszam + 1),
                    str(fazis),
        ...
        if tulmeretezes.get() == "Nem":
            ...
            # print('Nem méretez túl ' + str(mrmt))
    ...
    if len(inv_csomag) != 0:
        ...
        Label(csomagarak_frame, text=170 * "_", bg=bg, fg=fg).grid(
            row=0, column=0, columnspan=10, pady=5, sticky="w"
        )
        ...
        Label(
            ...
            fg="lime",
            font=csomag_text_font,
        ).grid(row=1, column=5, sticky="nsew", pady=2)
        Label(
            csomagarak_frame,
            text="Nettó\nszezonális\nkedv. ár(Ft)",
            bg=bg,
    ...
    for csomag_mfb in inv_csomag:
        ...
        nettoKedvezmenyesArF = (
            "{:,}".format(nettoKedvezmenyesAr).replace(",", " ") + " Ft"
        )
        ...
        # dict = {"a": kiirandoSorok[0], "b": mul(1, 3), "c": mul(1, 4)}
    ...
    Label(csomagarak_frame, text=107 * "=", bg=bg, fg=fg).grid(
        row=r, column=0, columnspan=10, sticky="w"
    )
...
# def polira_valtoztat(event):
#     arnyekos = arnyek.get()
#     pm = poli_mono.get()
#     if arnyekos == 'Igen' and pm == 'Monokristályos' or arnyekos == 'Igen' and pm == 'Polikristályos (Growatt)'
#     or arnyekos == 'Igen' and pm == 'Polikristályos (Fronius)':
#         poli_mono.set('Polikristályos (SolarEdge)')
#     # if arnyekos == 'Nem' and pm == 'Polikristályos':
#     #     poli_mono.set('Monokristályos')


# def pm(event):
#     arnyekos = arnyek.get()
#     pm = poli_mono.get()
#     if arnyekos == 'Igen' and pm == 'Monokristályos' or arnyekos == 'Igen' and pm != 'Polikristályos (SolarEdge)':
#         arnyek.set('Nem')
#     if arnyekos == 'Nem' and pm == 'Polikristályos (SolarEdge)':
#         arnyek.set('Igen')


Label(
    adatbekeres_frame,
    text="Áramszolgáltató:",
    font=text_font,
    bg=bg,
    fg=fg,
    width=23,
...
Label(
    adatbekeres_frame, text="Poli- vagy monokristályos:", bg=bg, fg=fg, font=text_font
...
# poli_mono.bind("<<ComboboxSelected>>", pm)
poli_mono.grid(row=6, column=1, pady=5, sticky="w")

Label(adatbekeres_frame, text="Fázisszám:", bg=bg, fg=fg, font=text_font).grid(
...
Label(adatbekeres_frame, text="Árnyékos hely:", bg=bg, fg=fg, font=text_font).grid(
...
arnyek.set("Nem")
# arnyek.bind("<<ComboboxSelected>>", polira_valtoztat)
arnyek.grid(row=8, column=1, pady=5, sticky="w")

Label(
    adatbekeres_frame, text="Tűzeseti leválasztó:", bg=bg, fg=fg, font=text_font
...
Label(adatbekeres_frame, text="Finanszírozás:", bg=bg, fg=fg, font=text_font).grid(
    row=10, column=0, padx=10, pady=5, sticky="w"
)

finanszirozas = Combobox(
    adatbekeres_frame, width=25, justify="center", state="readonly"
)
...
Label(
    adatbekeres_frame, text="Szezonális kedv. (%):", bg=bg, fg=fg, font=text_font
).grid(row=11, column=0, padx=10, pady=5, sticky="w")

szezonalis_kedv = Combobox(
    adatbekeres_frame, width=25, justify="center", state="readonly"
)
...
Label(adatbekeres_frame, text="Rendszer bővítése", bg=bg, fg=fg, font=text_font).grid(
...
kalkulal_gomb = Button(
    adatbekeres_frame,
    text="Rendszer kalkuláció",
    width=45,
    cursor="hand2",
    bg="green",
    fg="white",
...
kezdo_kep_frame = Frame(win, width=win_width / 2, bg=bg).grid(
    row=0, column=1, rowspan=18, sticky="n", padx=20, pady=3
)

kezdo_kep_frame2 = Frame(win, width=win_width, bg=bg).grid(
    row=2, column=0, columnspan=2, sticky="ws", padx=5, pady=3
)
...
def to_pdf():
    ...
    if str(neve.get()).strip() != "" and str(lakcim.get()).strip() != "":
        ...
        pdf.cell(
            190, 10, "Napelemes rendszer aj\u00E1nlat - HMKE", 0, 1, "C", fill=True
        )
        ...
        pdf.set_font("Times", "I", 10)
        pdf.cell(40, 10, latin_szoveg("Pista"), 0, 0, "R")
        ...
        pdf.cell(30, 10, "Tel.: ")
        pdf.set_font("Times", "I", 10)
        pdf.cell(40, 10, "06209697089", 0, 0, "R")
        pdf.ln(6)
        ...
        pdf.set_font("Times", "I", 10)
        pdf.cell(40, 10, "pistafiok@gmail.com", 0, 0, "R")
        pdf.ln(6)
        ...
        ervenyes = (
            "Az ajánlat visszavonásig érvényes. A feltüntetett adatok kizárólag tájékoztató jellegűek, "
        ...
        pdf.cell(
            ...
            5,
            latin_szoveg(
                "Kivételek téli időszakban történő kivitelezés esetén"),
            "B",
            0,
            "L",
        )
        ...
        kivetelek1 = (
            "Amennyiben a felmérés/kivitelezés/próbaüzem és átadás november 15. és március 15. közötti téli "
            "időszakra esik, úgy a Vállalkozó"
        )
        kivetelek2 = (
            "teljesítési határidejébe ezen időszak nem számít bele, tekintettel arra, hogy a tetőn történő "
            "biztonságos munkavégzés feltételei az"
        )
        kivetelek3 = (
            "időjárás miatt nem adottak. Amennyiben a felek, a Vállalkozó kezdeményezését követően, "
            "úgy ítélik meg, hogy a helyi időjárási"
        )
        kivetelek4 = (
            "körülmények engedik a biztonságos munkavégzést, úgy ezen téli időszak alatt is folytatható a "
        ...
        pdf.ln(5)
        pdf.cell(190, 5, latin_szoveg("Pista"), 0, 0, "C")
        pdf.ln(3)
